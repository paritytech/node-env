...
import { buildKusamaChainSpec, OMNI_NODE_BIN } from '../lib/chain_spec.ts'
import { serve } from '../lib/process.ts'

export interface KusamaOptions {
    mode?: string
    devBlockTime?: number
    log?: string
}

export async function kusama(opts: KusamaOptions = {}): Promise<void> {
    ...
    if (mode === 'build' || mode === 'default') {
        ...
        await buildKusamaChainSpec()
        if (mode === 'build') return
    }

    const chainSpec = join(CHAINSPEC_DIR, 'ah-kusama-spec.json')
...
export const kusamaCommand = new Command()
    .name('kusama')
    .description('Build or run kusama')
    ...
    .action((options, mode) => kusama({ mode: mode ?? 'default', ...options }))
